-- CPL Ver1.0: generic race-class support.
-- Race-class masters are scoped by racecourse so JRA and local racing can evolve independently.

alter table public.races
  add column if not exists race_class text;

-- JRA initial masters. Local racecourses intentionally have no entries yet.
with jra_racecourses(racecourse) as (
  values ('札幌'), ('函館'), ('福島'), ('新潟'), ('東京'),
         ('中山'), ('中京'), ('京都'), ('阪神'), ('小倉')
), race_classes(option_value, sort_order) as (
  values ('新馬', 10), ('未勝利', 20), ('1勝クラス', 30), ('2勝クラス', 40),
         ('3勝クラス', 50), ('リステッド', 60), ('オープン', 70),
         ('G3', 80), ('G2', 90), ('G1', 100)
)
insert into public.master_options (category, field_key, option_value, sort_order)
select 'RACE_CLASS', racecourse, option_value, sort_order
from jra_racecourses cross join race_classes
...
begin
  ...
  select exists (
    ...
    where category = 'RACE_CLASS'
      and field_key = p_race->>'racecourse'
      and active = true
  ) into v_has_race_class_master;

  if v_has_race_class_master then
    if v_race_class is null or not exists (
      select 1 from public.master_options
      where category = 'RACE_CLASS'
        and field_key = p_race->>'racecourse'
      ...
      raise exception 'INVALID_RACE_CLASS_MASTER';
    end if;
  elsif v_race_class is not null then
    raise exception 'INVALID_RACE_CLASS_MASTER';
  end if;
  ...
  if p_race_id is null then
    ...
    insert into public.races (
      ...
      distance, race_class, track_condition, field_size, created_by
    ) values (
