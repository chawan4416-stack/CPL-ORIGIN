(() => {
  ...
  function validate(race, results) { if (Object.entries(race).filter(([key]) => key !== 'race_class').some(([,value]) => value === '' || value == null || Number.isNaN(value)) || ($('race_class').required && !race.race_class)) return 'レース情報をすべて入力してください。'; if (!results.every(r => r.popularity > 0 && r.win_odds >= 1 && bodyFields.every(([,k]) => r[k]))) return '1〜3着の情報をすべて入力してください。'; return ''; }
