(() => {
  ...
  async function loadMasters() { const { data, error } = await client.from('master_options').select('category,field_key,option_value,sort_order').eq('active', true).order('sort_order'); if (error) throw new Error(`Master読み込み失敗: ${error.message}`); state.masters = {}; data.forEach(row => { const key = row.field_key; (state.masters[key] ||= []).push(row); }); $('racecourse').innerHTML = options('RACECOURSE'); $('surface').innerHTML = options('SURFACE'); $('track_condition').innerHTML = options('TRACK_CONDITION'); buildRaceAndFieldSizeOptions(); buildResultCards(); updateDistances(); updateCourseOptions(); renderMaster(); }
  function renderMaster() { const keys = ['RACECOURSE','SURFACE','TRACK_CONDITION','CHEST','HINDQUARTER','GAIT','BALANCE','TONE','ABDOMEN','PADDOCK_EVALUATION']; $('masterList').innerHTML = keys.map(k => `<div class="card master-card"><h2>${esc(k)}</h2><div class="chips">${(state.masters[k] || []).map(x => `<span>${esc(x.option_value)}</span>`).join('')}</div></div>`).join(''); }
  ...
  function collectResults() { return [...document.querySelectorAll('.result-card')].map(card => { const get = k => card.querySelector(`[data-field="${k}"]`).value; return { finish_position:Number(card.dataset.position), popularity:Number(get('popularity')), win_odds:Number(`${get('win_odds_int')}.${get('win_odds_dec')}`), chest:get('CHEST'), hindquarter:get('HINDQUARTER'), gait:get('GAIT'), balance:get('BALANCE'), tone:get('TONE'), abdomen:get('ABDOMEN'), paddock_evaluation:get('PADDOCK_EVALUATION') }; }); }
  function validate(race, results) { if (Object.values(race).some(v => v === '' || v == null || Number.isNaN(v))) return 'レース情報をすべて入力してください。'; if (!results.every(r => r.popularity > 0 && r.win_odds >= 1 && bodyFields.every(([,k]) => r[k]))) return '1〜3着の情報をすべて入力してください。'; return ''; }
  ...
  async function loadRecords() { $('recordsError').textContent = ''; const { data, error } = await client.from('races').select('id,race_date,racecourse,race_number,course,surface,distance,track_condition,field_size').order('race_date',{ascending:false}).order('race_number',{ascending:false}).limit(50); if (error) throw new Error(`登録データ取得失敗: ${error.message}`); if (!data?.length) { $('recordsList').innerHTML = '<div class="card empty-records">登録済みレースはありません。</div>'; return; } $('recordsList').innerHTML = data.map(r => `<div class="card record-card"><div class="record-main"><strong>${esc(r.race_date)}　${esc(r.racecourse)} ${esc(r.race_number)}R</strong><span>${esc(r.surface)} ${esc(r.distance)}m・${esc(r.course)}・${esc(r.track_condition)}・${esc(r.field_size)}頭</span></div><button class="delete-button" data-delete-race="${esc(r.id)}" type="button">削除</button></div>`).join(''); }
  ...
  $('racecourse').addEventListener('change', () => { updateDistances(); updateCourseOptions(); writeDraft(); });
